// Implement `TryFrom<String>` and `TryFrom<&str>` for the `TicketDescription` type,
...
#[error("{message}")]
pub struct InvalidTicketDescription {
    message: String,
}
...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(InvalidTicketDescription {
                message: "The description cannot be empty".into(),
            })
        }
        if value.len() > 500 {
            return Err(InvalidTicketDescription {
                message: "The description cannot be longer than 500 bytes".into(),
            })
        }
